...
const WhatsAppDashboardView = React.memo(
  ({
    metricas,
    serieHistorica,
    relatorioCampanhas,
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <FunnelWhatsApp
          metricas={metricas}
          onVendasChange={(v) =>
            setVendasLocais((prev: any) => ({ ...prev, whatsapp: v }))
          }
        />
      ...
      <CampanhasTable
        campanhasComMetricas={relatorioCampanhas}
        tipo="whatsapp"
      />
...
const LeadsDashboardView = React.memo(
  ({
    metricas,
    serieHistorica,
    relatorioCampanhas,
    ...
    const dadosCruzados = todasDatas.map((data) => ({
      data,
      qualificados:
        (leadsGrupos || []).find((l: any) => l.data === data)
          ?.leads_ensino_superior || 0,
      desqualificados:
        (leadsGrupos || []).find((l: any) => l.data === data)
          ?.leads_ensino_medio || 0,
      investimento:
        (serieHistorica || []).find((s: any) => s.data === data)
          ?.investimento || 0,
    }));
      ...
      <div className="space-y-10 animate-in fade-in duration-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <ClassificacaoTrafico
            ...
            isLoading={false}
          />
          <GruposWhatsApp value={gruposWhatsApp} onChange={setGruposWhatsApp} />
        </div>
...
export default function DashboardPublico() {
  ...
  const [vendasLocais, setVendasLocais] = React.useState<{
    [key: string]: number;
  }>({});
  const [gruposWhatsApp, setGruposWhatsApp] = React.useState({
    ensino_superior: 0,
    ensino_medio: 0,
  });
        ...
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-(--text-secondary) text-[14px]">
              O cliente não foi encontrado ou ocorreu um problema durante a
              extração de dados do Google Sheets. Verifique o link e a
              integração.
            </p>
          </div>
        </div>
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-10 space-y-10">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                {tipo === "whatsapp" && (
                  <span className="text-[11px] px-2 py-0.5 rounded-[4px] font-medium uppercase tracking-tight bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">
                    WhatsApp Performance
                  </span>
                )}
                {tipo === "leads" && (
                  <span className="text-[11px] px-2 py-0.5 rounded-[4px] font-medium uppercase tracking-tight bg-blue-500/10 text-blue-500 border border-blue-500/20">
                    Leads Generation
                  </span>
                )}
                {tipo === "ambos" && (
                  <span className="text-[11px] px-2 py-0.5 rounded-[4px] font-medium uppercase tracking-tight bg-purple-500/10 text-purple-500 border border-purple-500/20">
                    Híbrido
                  </span>
                )}
                <span className="text-[11px] text-(--text-tertiary) uppercase tracking-wide hidden sm:inline-block">
                  Tempo real: ON
                </span>
              </div>
            </div>
        ...
        {tipo === "ambos" && (
          <Tabs
            ...
            className="w-full border-none shadow-none bg-transparent animate-in fade-in duration-700"
          >
            <TabsList className="grid w-full grid-cols-2 max-w-[400px] mb-8 bg-muted border border-(--card-border) p-1 rounded-xl h-11 mx-auto lg:mx-0">
              <TabsTrigger
                value="whatsapp"
                className="rounded-lg data-[state=active]:bg-white data-[state=active]:text-black text-[13px] font-medium transition-all"
              >
                WhatsApp View
              </TabsTrigger>
              <TabsTrigger
            ...
            >
              <WhatsAppDashboardView
                {...data}
                setVendasLocais={setVendasLocais}
              />
            </TabsContent>
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-10 mt-12 pt-10 border-t border-(--card-border)">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 text-[12px] text-(--text-tertiary) uppercase tracking-wider font-medium">
          ...
          <div className="flex items-center gap-6">
            ...
            <p className="text-(--text-primary) font-semibold tracking-widest">
              Powered by DASHBOARD KV
            </p>
          </div>
        </div>
      </div>
