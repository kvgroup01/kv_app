...
import { LayoutDashboard, ExternalLink, Save, CheckCircle, Eye } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         <Card><CardContent className="h-64 p-6"><Skeleton className="w-full h-full" /></CardContent></Card>
         <Card><CardContent className="h-64 p-6"><Skeleton className="w-full h-full" /></CardContent></Card>
      </div>
      
      {/* Tabela */}
      <Card><CardContent className="h-80 p-6"><Skeleton className="w-full h-full" /></CardContent></Card>
    </div>
  );
}
...
export default function DashboardEditor() {
  ...
  const publicUrl = clienteLogado?.slug && lancamento?.slug 
    ...
    <div className="flex flex-col lg:flex-row h-[calc(100vh-100px)] gap-6 overflow-hidden">
      ...
      <Card className="w-full lg:w-1/3 flex flex-col h-full bg-card/80 backdrop-blur-sm shadow-xl border-border shrink-0">
        <CardHeader className="pb-4 border-b">
           ...
           <CardDescription>Personalize o dashboard para o seu cliente</CardDescription>
        </CardHeader>
           ...
           <div className="p-6 space-y-8">
              ...
              <section className="space-y-4">
                 <div className="flex items-center justify-between">
                   ...
                   <Badge variant={lancamento?.status === 'ativo' ? 'default' : 'secondary'} className={lancamento?.status === 'ativo' ? 'bg-green-500/10 text-green-500 hover:bg-green-500/20' : ''}>
                      {lancamento?.status === 'ativo' ? 'Ativo' : lancamento?.status === 'encerrado' ? 'Encerrado' : 'Rascunho'}
                   </Badge>
                 </div>
              ...
              <section className="space-y-4">
                 <div className="flex items-center justify-between mb-2">
                   <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Seções do dashboard</h3>
                 </div>
                 
                 <div className="space-y-3">
                    {(Object.entries(secoes) as [SecaoId, SecaoConfig][]).map(([key, config]) => (
                      <div key={key} className="flex items-center justify-between p-3 border rounded-lg bg-background group hover:border-primary/50 transition-colors">
                         <div className="flex-1 mr-4">
                            {editSecao === key ? (
                               <Input 
                                  ...
                                  onKeyDown={e => e.key === 'Enter' && setEditSecao(null)}
             ...
             <AlertDialog>
               ...
               <AlertDialogContent>
                 <AlertDialogHeader>
                   ...
                   <AlertDialogDescription>
                     Isso irá tornar o dashboard visível para o cliente no link público. Deseja continuar?
                   </AlertDialogDescription>
                 </AlertDialogHeader>
                 <AlertDialogFooter>
                   <AlertDialogCancel>Cancelar</AlertDialogCancel>
                   <AlertDialogAction onClick={publicarDashboard}>Sim, publicar agora</AlertDialogAction>
                 </AlertDialogFooter>
               </AlertDialogContent>
             </AlertDialog>
           )}
      ...
      <div className="flex-1 flex flex-col h-full bg-muted/10 rounded-xl overflow-hidden border">
        ...
        <div className="h-14 border-b bg-background/95 backdrop-blur-sm flex items-center justify-between px-6 shrink-0">
           <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span className="text-sm font-medium uppercase tracking-wider">Preview ao Vivo</span>
           </div>
           {lancamento?.status === 'ativo' && (
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-thin bg-[#0f0f0f]">
           <div className="max-w-6xl mx-auto space-y-6">
              ...
              <div className="mb-8">
                 <h2 className="text-3xl font-bold tracking-tight text-foreground">{nome}</h2>
                 <p className="text-muted-foreground">Preview de como os dados serão exibidos</p>
              </div>
              ...
              <div className="space-y-6">
                 ...
                 {secoes.cards_metricas.ativo && (
                   <section>
                      <h3 className="text-lg font-semibold mb-4 text-foreground">{secoes.cards_metricas.titulo}</h3>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                         {[
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {secoes.funil.ativo && (
                      <section>
                        <h3 className="text-lg font-semibold mb-4 text-foreground">{secoes.funil.titulo}</h3>
                        <Card className="h-64 flex items-center justify-center bg-card"><span className="text-muted-foreground text-sm">Visualização de Funil</span></Card>
                      </section>
                    )}
                    {secoes.grafico_investimento.ativo && (
                      <section>
                        <h3 className="text-lg font-semibold mb-4 text-foreground">{secoes.grafico_investimento.titulo}</h3>
                        <Card className="h-64 flex items-center justify-center bg-card"><span className="text-muted-foreground text-sm">Gráfico Linha/Barra</span></Card>
                      </section>
                    )}
                 </div>
                 ...
                 {secoes.tabela_campanhas.ativo && (
                   <section>
                      <h3 className="text-lg font-semibold mb-4 text-foreground">{secoes.tabela_campanhas.titulo}</h3>
                      <Card className="h-40 flex items-center justify-center bg-card"><span className="text-muted-foreground text-sm">Tabela de Campanhas</span></Card>
                   </section>
                 )}
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   {secoes.ranking_publicos.ativo && (
                      <section>
                         <h3 className="text-lg font-semibold mb-4 text-foreground">{secoes.ranking_publicos.titulo}</h3>
                         <Card className="h-40 flex items-center justify-center bg-card"><span className="text-muted-foreground text-sm">Ranking de Públicos</span></Card>
                      </section>
                   )}
                   {secoes.ranking_criativos.ativo && (
                      <section>
                         <h3 className="text-lg font-semibold mb-4 text-foreground">{secoes.ranking_criativos.titulo}</h3>
                         <Card className="h-40 flex items-center justify-center bg-card"><span className="text-muted-foreground text-sm">Ranking de Criativos</span></Card>
                      </section>
                   )}
                 </div>
                 ...
                 {secoes.grupos_whatsapp.ativo && (
                   <section>
                      <h3 className="text-lg font-semibold mb-4 text-foreground">{secoes.grupos_whatsapp.titulo}</h3>
                      <Card className="h-32 flex items-center justify-center bg-card border-green-500/20 bg-green-500/5">
                        <span className="text-green-500 font-medium text-sm">Progresso dos Grupos de WhatsApp</span>
                      </Card>
                   </section>
                 )}
                 ...
                 {secoes.visao_financeira.ativo && (
                   <section>
                      <h3 className="text-lg font-semibold mb-4 text-foreground">{secoes.visao_financeira.titulo}</h3>
                      <Card className="h-32 flex items-center justify-center bg-card">
                        <span className="text-muted-foreground font-medium text-sm">Métricas Financeiras (Vendas, Ticket Médio)</span>
                      </Card>
                   </section>
                 )}
