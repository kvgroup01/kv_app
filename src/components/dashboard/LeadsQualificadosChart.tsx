...
export function LeadsQualificadosChart({
  dados,
  isLoading,
}: LeadsQualificadosChartProps) {
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length >= 2) {
      ...
      const investimentoEntry = payload.find(
        (p: any) => p.dataKey === "investimento",
      );
      const qualificadosEntry = payload.find(
        (p: any) => p.dataKey === "leads_qualificados",
      );
      const desqualificadosEntry = payload.find(
        (p: any) => p.dataKey === "leads_desqualificados",
      );
