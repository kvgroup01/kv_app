...
export function CreativosGrid({ criativos, tipo, isLoading }: CreativosGridProps) {
  if (isLoading) {
    return (
  ...
  const isVideo = (name: string) => name.toLowerCase().includes('video') || name.toLowerCase().includes('vid');
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {criativos.map((criativo) => {
          ...
          if (criativo.performance === 'melhor') badgeClass = "bg-[#22c55e] text-white hover:bg-[#16a34a]";
          if (criativo.performance === 'bom') badgeClass = "bg-[#eab308] text-white hover:bg-[#ca8a04]";
              ...
              <div className="relative h-[160px] w-full bg-muted/30 border-b flex items-center justify-center">
                {criativo.thumbnail_url ? (
                  ...
                  <div className={cn("flex flex-col items-center justify-center w-full h-full", 
                    isVideo(criativo.nome) ? "bg-indigo-500/10 text-indigo-500" : "bg-blue-500/10 text-blue-500"
                  )}>
                    {isVideo(criativo.nome) ? <Play className="w-8 h-8 mb-2" /> : <ImageIcon className="w-8 h-8 mb-2" />}
                ...
                <div className="space-y-2">
                   ...
                   {tipo === 'whatsapp' ? (
                     <div className="flex justify-between items-center text-xs">
                       ...
                       <span className="font-semibold text-[#25D366]">{fmtNum(criativo.conversas)}</span>
                     </div>
                   ) : (
                     <>
                       <div className="flex justify-between items-center text-xs">
                       ...
                       <div className="flex justify-between items-center text-xs">
                         ...
                         <span className="font-semibold">{fmtPct(criativo.pct_qualificados)}</span>
                       </div>
                     </>
                   )}
                </div>
