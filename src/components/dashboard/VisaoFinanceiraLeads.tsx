...
export function VisaoFinanceiraLeads({
  ...
  const [inputValue, setInputValue] = React.useState(fmtBRL(investimentoManual));
  ...
  return (
    <Card>
      <CardHeader className="pb-4">
        ...
        <CardDescription>Acompanhamento e controle do budget injetado x utilizado</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        
        {/* Linha 1 - Input */}
        <div className="space-y-2">
          <label className="text-sm font-medium">Investimento Total na Plataforma</label>
          <Input 
             type="text" 
             value={inputValue}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 bg-muted/30 p-4 rounded-xl border">
          <div className="space-y-1">
             <div className="text-sm text-muted-foreground mb-1">Taxa Facebook Ads</div>
             <Badge variant="outline" className="font-semibold">{fmtBRL(taxaData.taxa)} (12,5%)</Badge>
          </div>
          <div className="space-y-1">
             <div className="text-sm text-muted-foreground">Valor Real Investido</div>
          ...
          <div className="space-y-1 col-span-2 lg:col-span-1 border-t lg:border-t-0 lg:border-l border-border pt-3 lg:pt-0 lg:pl-4">
             ...
             <div className="text-lg font-bold">{fmtBRL(taxaData.valorComTaxa)}</div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 py-2">
          <div className="space-y-1">
             <div className="text-sm text-muted-foreground uppercase tracking-widest">Valor Utilizado</div>
             <div className="text-2xl font-semibold">{fmtBRL(valorUsadoCampanhas)}</div>
          </div>
          
          <div className="space-y-1 sm:text-right">
             <div className="text-sm text-muted-foreground uppercase tracking-widest">Saldo Restante</div>
